...
GAIA_IN = "/mnt/g/STAR_HPV/raw/gaia_dr3/gaia_legacy_ids_besttable.parquet"
...
KIC2GAIA = "/mnt/g/STAR_HPV/raw/ages/legacy_2017_kic_to_gaia_sourceid_fixed.csv"
KEP_GMM = "/mnt/g/STAR_HPV/results/gate/kepler_gmm_gate.yaml"
MCQ = "/mnt/g/STAR_HPV/raw/kepler/mcquillan2014_rotators.parquet"

OUT = "/mnt/g/STAR_HPV/processed/state_vectors/legacy_led_state_vector_besttable.parquet"
SUM = "/mnt/g/STAR_HPV/results/state_vectors/summary_legacy_led_besttable.yaml"
...
def main():
    ...
    # IMPORTANT: drop Gaia photometry columns from idmap to avoid suffixing collisions
    idmap = pd.read_csv(KIC2GAIA, dtype={"source_id":"string"}).copy()
    drop_cols = [c for c in ["ra","dec","phot_g_mean_mag","ruwe","dist_deg"] if c in idmap.columns]
    if drop_cols:
        idmap = idmap.drop(columns=drop_cols)
    ...
    if c_plx is None or c_G is None:
        raise RuntimeError(f"Missing parallax/Gmag after merge. cols={list(df.columns)}")

    df["prot_days"] = pd.to_numeric(df["prot_days"], errors="coerce")
    df = df.dropna(subset=["prot_days"]).copy()
    df = df[df["prot_days"] > 0].copy()

    teff = pd.to_numeric(df["Teff"], errors="coerce").fillna(pd.to_numeric(df["teff_gspphot"], errors="coerce"))
    df["Teff_use"] = teff
