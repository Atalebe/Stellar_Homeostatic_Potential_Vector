...
STATE_PARQ = "/mnt/g/STAR_HPV/processed/state_vectors/gaia_shv_state_vector_gateaware_v2.parquet"
...
def main():
    ...
    for tb, binfo in nm["bins"].items():
        ...
        f_win_slow = float(sub.loc[slow, "in_window_v2"].mean()) if slow.any() else None
        f_win_fast = float(sub.loc[fast, "in_window_v2"].mean()) if fast.any() else None
